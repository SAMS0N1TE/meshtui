...
async def connect_tcp(app, iface, cfg):
    """Show a dialog to connect to a TCP host."""
    host = await input_dialog(
        title="TCP Connection",
        text="Enter hostname or IP address:",
        default=cfg.last_port if cfg and (":" in cfg.last_port or "." in cfg.last_port) else "meshtastic.local"
    ).run_async()

    if host:
        cfg.last_port = host
        cfg.save()
        iface.start(port=host)
        app.invalidate()

async def show_connection_error_dialog(app, iface, cfg, port, error):
    """Display a connection error dialog with options."""
    choice = await radiolist_dialog(
        title="Connection Error",
        text=f"Failed to connect to '{port}'.\n\nError: {error}\n\nWhat would you like to do?",
        values=[
            ("retry", "Try Again"),
            ("reconfigure", "Configure Different Port"),
...
async def connect_port(app, state, iface, cfg) -> Optional[str]:
    ...
    baud_choice = await radiolist_dialog(
        title="Baud Rate",
        text="Select baud rate setting:",
        values=[("auto", "Automatic"), ("manual", "Manual")],
    ).run_async()

    if baud_choice == "manual":
        baud_rate_str = await input_dialog(
            title="Manual Baud Rate",
            text="Enter baud rate:",
        ).run_async()
        try:
            cfg.baud_rate = int(baud_rate_str)
...
async def setup_wizard(app, state, iface, cfg) -> None:
    ...
    # Port
    await connect_port(app, state, iface, cfg)
